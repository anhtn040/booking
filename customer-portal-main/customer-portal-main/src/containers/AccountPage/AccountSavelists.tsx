...
import { getFavorites } from "apis/auth/me/meApi";
...
const AccountSavelists = () => {
  ...
  const [favorites, setFavorites] = useState({} as IResidenceResponse)
          ...
          <Tab.Group>
            ...
            <Tab.Panels>
              <Tab.Panel className="mt-8">
                ...
                <div className="grid grid-cols-1 gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {favorites.data?.length > 0 && favorites.data.map((residence) => (
                    <HotelBanner key={residence.residenceId} tym={true}
                      href={`/residences/${residence.residenceId}?checkin=${getDateTomorrowFormatYYYYMMDD()}&checkout=${getDateAfterTomorrowFormatYYYYMMDD()}`} data={residence}/>
                  ))}
                </div>
                {(favorites.data?.length === 0 || favorites.data === undefined) && 
                  (<div className="flex justify-center">
                    <NcImage src={imageNotFound} />
                  </div>
                )}
